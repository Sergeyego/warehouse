...
void FormBalanceEW::startUpd()
{
    ...
    if (en_el){
        ...
        query=QString("select 'w:'||p.id, pr.nam || CASE WHEN p.id_var<>1 THEN ' /'||ev.nam ||'/' ELSE '' END, d.sdim, k.short|| "
                        "CASE WHEN wp.id<>0 THEN (' ('||wp.mas_ed ||' кг'||(case when wp.pack_group<>'-' then '/'||wp.pack_group||')' else ')' end)) ELSE '' END, "
                        "m.n_s||'-'||date_part('year',m.dat), m.dat, "
                        "s.nam, pb.n_plav, p.prim_prod, c.st, "
                        "pr.nam||CASE WHEN p.id_var<>1 THEN ' /'||ev.nam ||'/' ELSE '' END||'#'||d.sdim||'#'||k.short|| "
                        "CASE WHEN wp.id<>0 THEN (' ('||wp.mas_ed ||' кг'||(case when wp.pack_group<>'-' then '/'||wp.pack_group||')' else ')' end)) ELSE '' END "
                        "from wire_parti p "
